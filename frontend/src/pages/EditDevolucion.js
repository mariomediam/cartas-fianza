...
const EditDevolucion = () => {
  ...
  const loadInitialData = async () => {
    setLoadingData(true);
    try {
      // Cargar datos del historial de devolución
      const historyRes = await api.get(`/warranty-histories/${warrantyHistoryId}/`);
      const historyData = historyRes.data;
      setOriginalData(historyData);
      
      // Cargar el historial previo (la carta activa antes de la devolución)
      ...
      if (warranty.history && warranty.history.length > 1) {
        // Buscar el historial previo al de devolución
      ...
      toast.error('Error al cargar la información de la devolución');
      navigate(-1);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Enviar al backend
      await api.post(`/warranty-histories/${warrantyHistoryId}/modificar-devolucion/`, formDataToSend, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      
      toast.success('Devolución modificada correctamente');
      navigate('/cartas-fianza', { state: { shouldRefresh: true } });
      
    } catch (error) {
      console.error('Error al modificar la devolución:', error);
      ...
      if (error.response?.data?.error) {
        ...
        if (typeof errors === 'object') {
          ...
          toast.error('Error al modificar la devolución');
        }
      } else {
        toast.error('Error al modificar la devolución');
      }
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="relative flex items-start gap-3">
          ...
          <div className="flex-1 pr-12">
            <h1 className="text-2xl font-bold text-gray-900">Modificar devolución</h1>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm p-6 space-y-6">
          ...
          <div className="relative py-4">
            ...
            <div className="relative flex justify-start">
              <span className="bg-white pr-4 text-lg font-semibold text-green-700">
                Devolución
              </span>
            </div>
          </div>
