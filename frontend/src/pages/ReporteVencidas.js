...
const ReporteVencidas = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const response = await api.get('/warranties/vencidas-por-fecha/', { params });
      setResults(response.data);

      if (response.data.count === 0) {
        toast.info('No se encontraron cartas vencidas con los filtros aplicados');
      } else {
        toast.success(`Se encontraron ${response.data.count} carta(s) vencida(s)`);
      }
    } catch (error) {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>Reporte de Cartas Fianza Vencidas</title>
        <style>
          @page {
            size: A4 landscape;
            margin: 10mm;
      ...
      <body>
        <div class="header-container">
          ...
          <div class="header-title">
            <h1>REPORTE DE CARTAS FIANZA VENCIDAS</h1>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              Reporte de Cartas Fianza Vencidas
            </h1>
            <p className="text-gray-600 mt-1">
              Consulta las cartas fianza vencidas a una fecha específica
            </p>
          </div>
        </div>
          ...
          <form onSubmit={handleSearch}>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                type="submit"
                disabled={loading}
                className="inline-flex items-center justify-center px-6 py-2.5 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 focus:ring-4 focus:ring-primary-300 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
                    <svg
...
export default ReporteVencidas;
