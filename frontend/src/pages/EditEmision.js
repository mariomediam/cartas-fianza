...
const EditEmision = () => {
  ...
  // Estado para los datos originales (solo lectura)
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      const [letterTypesRes, financialEntitiesRes, currencyTypesRes, historyRes] = await Promise.all([
      ...
      setFormData({
        letter_type: historyData.letter_type_id || '',
        ...
        contractor: historyData.contractor_id ? {
          value: historyData.contractor_id,
          label: `${historyData.contractor_ruc} - ${historyData.contractor_business_name}`,
        } : null,
      ...
      toast.error('Error al cargar la información de la emisión');
      navigate(-1);
    } finally {
  ...
  // Manejar cambio de contratista
  const handleContractorChange = (selectedOption) => {
    setFormData(prev => ({ ...prev, contractor: selectedOption }));
  };
  ...
  const validateForm = () => {
    if (!formData.letter_type) {
      toast.error('Debe seleccionar el tipo de carta');
      return false;
    }
    ...
    if (!formData.reference_document.trim()) {
      toast.error('Debe ingresar el documento de referencia');
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append('contractor', formData.contractor.value);
      ...
      // Enviar al backend
      await api.post(`/warranty-histories/${warrantyHistoryId}/modificar-emision/`, formDataToSend, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      
      toast.success('Emisión modificada correctamente');
      navigate('/cartas-fianza', { state: { shouldRefresh: true } });
      
    } catch (error) {
      console.error('Error al modificar la emisión:', error);
      ...
      if (error.response?.data?.error) {
        ...
        if (typeof errors === 'object') {
          ...
          toast.error('Error al modificar la emisión');
        }
      } else {
        toast.error('Error al modificar la emisión');
      }
  ...
  const handleContractorCreated = async () => {
    toast.success('Contratista agregado. Puede seleccionarlo ahora.');
  };
    ...
    <Layout>
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="relative">
          <div className="pr-12">
            <h1 className="text-2xl font-bold text-gray-900">Modificar emisión</h1>
