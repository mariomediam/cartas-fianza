...
const CurrencyTypes = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const url = searchTerm.trim()
        ? `/currency-types/?search=${encodeURIComponent(searchTerm.trim())}`
        : '/currency-types/?page_size=1000';
      
      const response = await api.get(url);
      const data = response.data.results || response.data;
      setCurrencyTypes(data);
    } catch (error) {
      console.error('Error al buscar tipos de moneda:', error);
      toast.error('Error al buscar los tipos de moneda');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.symbol.trim()) {
      toast.error('El símbolo es obligatorio');
      return;
    }
    ...
    try {
      ...
      if (editingCurrency) {
        await api.put(`/currency-types/${editingCurrency.id}/`, dataToSend);
        toast.success('Tipo de moneda actualizado correctamente');
      } else {
        await api.post('/currency-types/', dataToSend);
        toast.success('Tipo de moneda creado correctamente');
      }
      ...
      if (error.response?.data) {
        ...
        toast.error('Error al guardar el tipo de moneda');
      }
    }
  };
  ...
  const confirmDelete = async () => {
    try {
      await api.delete(`/currency-types/${deletingCurrency.id}/`);
      toast.success('Tipo de moneda eliminado correctamente');
      ...
      if (error.response?.status === 400 || error.response?.status === 409) {
        ...
        toast.error('Error al eliminar el tipo de moneda');
      }
    }
  };
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">
            {searchTerm ? `No hay tipos de moneda que coincidan con "${searchTerm}".` : 'No hay tipos de moneda registrados.'}
          </p>
        </div>
      ) : (
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-900">
                {editingCurrency ? 'Editar Tipo de Moneda' : 'Agregar Tipo de Moneda'}
              </h2>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-primary-500 focus:border-transparent placeholder-gray-400 uppercase"
                    placeholder="Ej: PEN"
                    maxLength={3}
                    required
                  />
                  <p className="mt-1 text-xs text-gray-500">
                    Código ISO de 3 caracteres (se convertirá a mayúsculas)
                  </p>
                </div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                >
                  {editingCurrency ? 'Actualizar' : 'Guardar'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-sm text-gray-600 text-center mb-6">
              ¿Estás seguro de que deseas eliminar "{deletingCurrency?.description} ({deletingCurrency?.code})"? Esta acción no se puede deshacer.
            </p>
