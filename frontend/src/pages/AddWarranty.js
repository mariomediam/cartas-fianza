...
const AddWarranty = () => {
  ...
  const loadCatalogs = async () => {
    ...
    try {
      const [letterTypesRes, financialEntitiesRes, currencyTypesRes] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append('warranty_object', warrantyObjectId);
      ...
      formDataToSend.append('warranty_status', '1'); // Estado inicial: Vigente
      ...
      // Enviar al backend
      await api.post('/warranties/', formDataToSend, {
        headers: {
      ...
      console.error('Error al guardar la carta fianza:', error);
      ...
      if (error.response?.data) {
        ...
        if (typeof errors === 'object') {
          ...
          toast.error('Error al guardar la carta fianza');
        }
      } else {
        toast.error('Error al guardar la carta fianza');
      }
  ...
  const handleContractorCreated = async () => {
    // Recargar la lista de contratistas si es necesario
    ...
    <Layout>
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm p-6 space-y-6">
          ...
          {/* Contratista (fuera del grid para darle más espacio) */}
          <div>
