...
const ViewDevolution = () => {
  ...
  const loadDevolutionData = useCallback(async () => {
    ...
    try {
      ...
      // Buscar el historial anterior a la devolución (el último con estado activo)
      if (warranty.history && warranty.history.length > 1) {
        // Ordenar por ID descendente
        const sortedHistory = [...warranty.history].sort((a, b) => b.id - a.id);
        // El historial anterior es el segundo (el primero es la devolución actual)
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="relative flex items-start gap-3">
          ...
          <div className="flex-1 pr-12">
            <h1 className="text-2xl font-bold text-gray-900">Ver devolución</h1>
            {devolutionData.warranty_object_description && (
              <p className="text-gray-600 mt-1 uppercase font-medium">{devolutionData.warranty_object_description}</p>
            )}
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 space-y-6">
          ...
          {/* ===== SECCIÓN DE DEVOLUCIÓN (Solo lectura) ===== */}
