...
const ViewExecutionModal = ({ isOpen, onClose, warrantyHistoryId, onDeleted }) => {
  ...
  const loadExecutionData = useCallback(async () => {
    ...
    try {
      ...
      // Buscar el historial anterior a la ejecución
  ...
  }, [isOpen, warrantyHistoryId, loadExecutionData]);
  ...
  const handleModificar = () => {
    onClose();
    navigate(`/cartas-fianza/editar-ejecucion/${warrantyHistoryId}`);
  };
        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden transform transition-all">
          ...
          <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900">Ver ejecución</h2>
                {executionData?.warranty_object_description && (
                  <p className="text-gray-600 text-sm uppercase font-medium">{executionData.warranty_object_description}</p>
                )}
              </div>
            </div>
            <button
...
export default ViewExecutionModal;
